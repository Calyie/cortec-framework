"""Run cortec end to end on your own table: Stage A (release), Stage B (generate), Stage C
(bound), then the evaluation beside real references. Every stage prints in the standard layout
and every file goes under one folder, named at the end.
  ...
  python3 run_cortec.py --data private.csv --holdout holdout.csv --schema my_schema.py \\
...
release.json there; a later run with the same folder reuses it, because a release cannot be
...
ap.add_argument("--pool-factor", type=int, default=1,
                help="1 (default): generate exactly n rows; 3: generate 3n and select the n that match the "
                     "release best, the README's recommendation, at three times the spend")
